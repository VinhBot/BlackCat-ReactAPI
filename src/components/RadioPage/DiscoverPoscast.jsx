...
const DiscoverPoscast = memo(({ data }) => {
   return (
      <PlayListSelector title={data?.title}>
         {data?.items?.length > 0 && data?.items.map((e, index) => {
            ...
            return <CarouselItem isHiddenButton={true} isSwiper={true} key={e.encodeId} artis={false} desc={false} class1={classGird} item={e} />
         })}
...
export default DiscoverPoscast;
